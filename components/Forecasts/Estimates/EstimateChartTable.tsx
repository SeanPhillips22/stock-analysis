...
export function EstimateChartTable({ title, data, type, lastActualDate }: Props) {
	...
	return (
		<div className="mt-3 overflow-x-auto p-0 text-center sm:p-0.5 lg:mt-3.5">
			<table className="w-full text-right text-tiny xs:text-sm md:text-small" id="ratings-table">
				<thead>
				...
				<tbody>
					{arr.map(i => {
						return (
							<tr className="border-b border-gray-200 last:border-0 hover:bg-gray-50" key={i}>
								<td className="whitespace-nowrap px-1 py-[3px] text-left">{capitalize(i)}</td>
								{dates.map((date: string) => {
									let d = tableData[date][i]

									let formatted: any
									if (type === 'revenue') formatted = abbreviateNumber(d, 1)
									else if (type.includes('Growth')) formatted = addColor(d)
									else formatted = format2dec(d)

									let title: any
									if (type === 'revenue') title = abbreviateNumber(d, 1)
									else if (type.includes('Growth')) title = format2dec(d) + '%'
									else title = format2dec(d)

									return (
										<td className="px-1 py-[3px]" title={title} key={date + i}>
											{formatted}
										</td>
									)
								})}
