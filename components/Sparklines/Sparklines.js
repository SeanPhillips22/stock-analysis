...
class Sparklines extends PureComponent {
	...
	render() {
		...
		const points = dataToPoints({ previousClose, data, limit, width, height, margin, max, min })

		const prevCloseCoords = points.pop()
		data.pop()
		...
		return (
			<svg {...svgOpts}>
				{React.Children.map(this.props.children, function (child) {
					return React.cloneElement(child, { prevCloseCoords, data, points, width, height, margin })
				})}
			</svg>
		)
	}
